    ...
    <div class="card-body">
        <form class="card-form">
            <div class="row align-items-center mb-10-none">
                ...
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list two">
                           <li class="one">{{ __("Rate") }}: <span>1 {{ get_default_currency_code() }} = {{ get_amount(@$data->currency->rate, @$data->currency->currency_code,$precision) }}</span></li>
                           @php
                                $conversionAmount = $data->request_amount * $data->currency->rate;
                         @endphp
                           <li class="two">{{ __("After Conversion") }}: <span>{{ get_amount(@$conversionAmount,@$data->currency->currency_code,$precision) }}</span></li>
                        <li class="three">{{ __("Total Charge") }}: <span>{{ get_amount(@$data->charge->total_charge,@$data->currency->currency_code,$precision) }}</span></li>

                        <li class="four">{{ __("Will Get") }}: <span>{{ get_amount(@$data->payable,@$data->currency->currency_code,$precision) }}</span></li>
